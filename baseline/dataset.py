...
class MaskBaseDataset_AddData(Dataset):
    num_classes = 3 * 2 * 3
    ...
    def __init__(self, data_dir, mean=(0.548, 0.504, 0.479), std=(0.237, 0.247, 0.246), val_ratio=0.2):
        ...
        self.transform1 = Compose([         
            CenterCrop((320, 256)),
            RandomHorizontalFlip(p=1.0), # randomly H_flip images
            Resize([512,384], Image.BILINEAR),
            ToTensor(),
            Normalize(mean=self.mean, std=self.std),
            AddGaussianNoise(), 
    ...
    def setup(self):
        ...
        for profile in profiles:
            ...
            for file_name in os.listdir(img_folder):
                ...
                # Old case: 
                if age_label is AgeLabels.OLD:
                    old.append([img_path,mask_label,gender_label,age_label])

        for i_path, i_mask, i_gender, i_age in old:
            self.image_paths.append(i_path)
            self.mask_labels.append(i_mask)
            self.gender_labels.append(i_gender)
            self.age_labels.append(i_age)
        for i_path, i_mask, i_gender, i_age in old:
            self.image_paths.append(i_path)
            self.mask_labels.append(i_mask)
            self.gender_labels.append(i_gender)
    ...
    def __getitem__(self, index):
        ...
        if index <= self.change_section:
            image_transform = self.transform(image)
        elif index > self.change_section and int((len(self) - self.change_section) /2 + self.change_section) >= index :
            image_transform = self.transform1(image)
        else:
            image_transform = self.transform2(image)
